...
@pytest.mark.django_db()
def test_05_get_comment_docs(user_client, create_comment):
...
@pytest.mark.django_db()
def test_06_comment_doc_create_without_file(user_client, create_comment):
    task, comment = create_comment
    data = {'text': 'asdf'}
...
@pytest.mark.django_db()
def test_07_comment_doc_create_with_file(user_client, create_comment):
    doc_file = create_text_file()
    task, comment = create_comment
    data = {'text': 'asdf',
            'doc_file': doc_file}
...
@pytest.mark.django_db()
def test_08_comment_doc_create_with_wrong_file_type(user_client,
                                                    create_comment):
    image = create_image_file()
    task, comment = create_comment
    data = {'text': 'asdf',
