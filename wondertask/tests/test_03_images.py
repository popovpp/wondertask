...
@pytest.mark.django_db()
def test_06_get_comment_image_list(user_client, create_comment):
...
@pytest.mark.django_db()
def test_07_get_comment_single_image(user_client, create_comment):
    task, comment = create_comment
...
@pytest.mark.django_db()
def test_08_comment_image_create_without_file(user_client, create_comment):
    task, comment = create_comment
    data = {}
    response = user_client.post(
        f'/v1/tasks/task/{task["id"]}/comment/{comment["id"]}/image/',
...
@pytest.mark.django_db()
def test_09_comment_image_create_and_delete_with_right_file_type(user_client,
                                                                 create_comment):
    image = create_image_file('small')
    task, comment = create_comment
    data = {'image_file': image}
    ...
    image_id = Image.objects.all()[0].id
    response2 = user_client.delete(
        f'/v1/tasks/task/{task["id"]}/comment/{comment["id"]}/image/{image_id}/')
    assert response2.status_code == 204, f'{response.json()}'
    assert os.path.isfile(filepath) is False


@pytest.mark.django_db()
def test_10_task_image_update_change_uploaded_file(user_client,
                                                   create_comment):
    image = create_image_file('small')
    task, comment = create_comment
    data = {'image_file': image}
...
@pytest.mark.django_db()
def test_11_comment_image_create_with_wrong_file_type(user_client,
                                                      create_comment):
