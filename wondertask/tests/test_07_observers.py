...
@pytest.mark.django_db()
def test_02_get_task_single_observer(user_client, create_task):
    task = create_task
    data = {"observer": 1}
...
@pytest.mark.django_db()
def test_03_task_observers_create(user_client, create_task):
    task = create_task
    data = {"observer": 1}
...
@pytest.mark.django_db()
def test_04_task_observers_delete_file_with_object(user_client,
                                                   create_task):
    task = create_task
    data = {"observer": 1}
    user_client.post(f'/v1/tasks/task/{task["id"]}/observers/', data=data)
